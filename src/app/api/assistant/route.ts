...
export async function POST(request:Request){
  ...
  const[trips,accounts,suppliers,invoices,payments,expenses,payroll]=await Promise.all([
    ...
    supabase.from("account_balances").select("id,code,name,account_type,balance").order("code").limit(300),
    supabase.from("suppliers").select("id,code,name,payable_account_id,is_active").order("name").limit(300),
    ...
    supabase.from("expense_transactions").select("expense_date,description,expense_account_id,amount,status").gte("expense_date",from).lte("expense_date",to).limit(500),
  ...
  const failed=[trips,accounts,suppliers,invoices,payments,expenses,payroll].find(result=>result.error);if(failed?.error)return NextResponse.json({error:`Could not prepare the report: ${failed.error.message}`},{status:500});
  const snapshot={period:{from,to},trips:trips.data??[],account_balances:accounts.data??[],suppliers:suppliers.data??[],supplier_invoices:invoices.data??[],supplier_payments:payments.data??[],expenses:expenses.data??[],payroll_runs:payroll.data??[]};
